...
class KafkaConfig(private val env: Map<String, String>) {
    ...
    fun kafkaConsumer(topic: String, groupId: String): KafkaConsumer<String?, ByteArray> {
        ...
        consumer.subscribe(Collections.singleton(topic), object : ConsumerRebalanceListener {
            override fun onPartitionsRevoked(partitions: Collection<TopicPartition?>) {
                partitions.forEach { tp -> logger.info("Rebalance: no longer assigned to topic {}, partition {}", tp?.topic(), tp?.partition()) }
            }

            override fun onPartitionsAssigned(partitions: Collection<TopicPartition?>) {
                partitions.forEach { tp -> logger.info("Rebalance: assigned to topic {}, partition {}", tp?.topic(), tp?.partition()) }
            }
        })
    ...
    private fun kafkaConsumerProperties(groupId: String): Map<String, Any> {
        val clientId = "pam-statistikk-data-consumer"
