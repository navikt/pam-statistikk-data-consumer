...
class TxTemplate(private val ds: DataSource) {
    ...
    fun <R> doInTransactionNullable(existingContext: TxContext? = null, txBlock: (ctx: TxContext) -> R): R? {
    ...
    fun <R> doInTransaction(existingContext: TxContext? = null, txBlock: (ctx: TxContext) -> R): R {
        val msg = "Uventet resultat, mottok null. Antageligvis programmeringsfeil, vurder å bruk doInTransaction direkte."
        return doInTransactionNullable(existingContext, txBlock) ?: throw IllegalStateException(msg)
    }
